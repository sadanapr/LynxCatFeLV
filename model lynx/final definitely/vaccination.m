function vaccination()
    ...
    param.vac_L = 1/90; %VALUE !!! (percentage vaccinated each day) 0.03 = 1/month
    
    %Model
    X0 = [param.S_L0 param.I_L0 param.S_C0 param.I_C0 param.R_C0 param.R0 param.V_L0];
...
%     legend show;
    ...
    plot(t,X(7,:),'y','DisplayName','Vaccinated Lynx');
    plot(t,X(1,:) + X(2,:) + X(7,:),'b','DisplayName','Lynx');
    xlim([0 365*13])
    xlabel('Time (days)')
    ylabel('Population')
    hold off;
...
%     %plot LCR
%     figure
%     plot(t,X(1,:) + X(2,:) + X(7,:),'r','DisplayName','L');
%     hold on;
%     plot(t,X(3,:)+X(4,:)+X(5,:),'b','DisplayName','C');
%     plot(t,X(6,:),'g','DisplayName','R');
%     hold off;
...
% function that computes the right-hand side of the differential equation
    ...
    dxdt = [  (param.e_LC * C * S_L/(C + 60)*(1 - L/param.K_L)) +  (param.e_LR * R * S_L/(R + 48)*(1 - L/param.K_L)) + (param.gamma_L * I_L) - (param.x_L * S_L) - (param.vac_L * S_L) - (param.beta_L * I_L * S_L/L) - (param.beta_CL * I_C * S_L/L), ... dS_L/dt
              ...
              (param.e_LC * C * V_L/(C + 60)*(1 - L/param.K_L)) + (param.e_LR * R * V_L/(R + 48)*(1 - L/param.K_L)) + (param.vac_L * S_L)  - (param.x_L * V_L) ... dV_L/dt
           ];
	dxdt = dxdt (:);
end