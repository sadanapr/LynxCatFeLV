function isolation()
    ...
    param.K_R = 2000;
    param.K_L = 800;
    param.isol_L = 1/30; %isolated infected lynx each day
    
    param.isoI_L = 0;
    
    %Model
    X0 = [param.S_L0 param.I_L0 param.S_C0 param.I_C0 param.R_C0 param.R0 param.isoI_L];
...
% function that computes the right-hand side of the differential equation
   ...
   dxdt = [  (param.e_LC * C * S_L/(C + 60)*(1 - L/param.K_L)) +  (param.e_LR * R * S_L/(R + 48)*(1 - L/param.K_L)) + (param.gamma_L * I_L) - (param.x_L * S_L) - (param.beta_L * I_L * S_L/L) - (param.beta_CL * I_C * S_L/L), ... dS_L/dt
              (param.e_LC * C * I_L/(C + 60)*(1 - L/param.K_L)) + (param.e_LR * R * I_L/(R + 48)*(1 - L/param.K_L)) + (param.beta_L * I_L  *  S_L/L) + (param.beta_CL * I_C * S_L/L) - (param.x_L * I_L) - (param.gamma_L * I_L) - (param.delta_L * I_L) - (param.isol_L* I_L/L), ... dI_L/dt
              ...
              (param.isol_L*I_L/L) - (param.delta_L * Q_L)%quarantined
           ];
	dxdt = dxdt (:);
end